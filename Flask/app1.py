...
#extractor page
@app.route('/keywords',  methods=['GET','POST'])# route to show the predictions in a web UI
def keywords():
    ...
    for i in range(0, len(distances.flatten())):
        if i == 0:
            print('Recommendations for {0}:\n'.format(us_canada_user_rating_pivot.index[output]))
        else:
            keyword.append('{0}: {1}'.format(i, 
                                us_canada_user_rating_pivot.index[indices.flatten()[i]]))
    
   # showing the prediction results in a UI
    return render_template('keywords.html',keywords=keyword)
